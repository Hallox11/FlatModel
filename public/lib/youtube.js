...
const YouTube = function () {
  ...
  */
   ...
   * @param {function} callback
   */
  self.getChannelById = function (mode, userId, tag, order, page,  callback) {
    var url;
  
   // var key ="AIzaSyBDYioRbkHEDV-NVUhqyN3XV2_rPOU1AiU";
    var key ="AIzaSyCfwmh5vv_gun23y-_iJCIs3-2mdfze6zM";
   /////////
    if(mode==="userChannel")
    {
    if(page==="") url="https://www.googleapis.com/youtube/v3/playlistItems?key="+key
                       +"&part=snippet&playlistId="+userId+"&order="+order+"&maxResults=50";
    else
                  url="https://www.googleapis.com/youtube/v3/playlistItems?key="+key
                        +"&part=snippet&pageToken="+page+"&playlistId="+userId+"&order="+order+"&maxResults=50";
    }
   ////////////
   if(mode==="userLists")
   {
   if(page==="")  url="https://www.googleapis.com/youtube/v3/playlists?key="+key
                      +"&part=snippet&part=contentDetails&channelId="+userId+"&order="+order+"&maxResults=50";
   else
                  url="https://www.googleapis.com/youtube/v3/playlists?key="+key
                      +"&part=snippet&part=contentDetails&pageToken="+page+"&channelId="+userId+"&order="+order+"&maxResults=50";
   }

    if(mode==="tag")
    {
    if(page==="") url="https://www.googleapis.com/youtube/v3/search?q="+tag+"&key="+key+"&part=snippet&order=relevance&maxResults=50";
    else
       url="https://www.googleapis.com/youtube/v3/search?q="+tag+"&key="+key+"&pageToken="+page+"&part=snippet&order=relevance&maxResults=50";
    }
      ...
      Object.keys(params).forEach((paramKey) => {
        if (params[paramKey] !== undefined) {
          self.addParam(paramKey, params[paramKey]);
           ...
           console.log("TAG URL: " + self.getUrl('search'));
  ...
  */
  ...
  self.getMostPopular = function (maxResults, callback) {
    ...
    if (validate !== null) {
      ...
      self.addParam('part', self.getParts());
      self.addParam('maxResults', maxResults);
      self.addParam('fields', 'items(kind,id,snippet(description,channelTitle,channelId,title,thumbnails))');
      self.addParam('chart', 'mostPopular');

      console.log("URL: " + self.getUrl('videos'));
